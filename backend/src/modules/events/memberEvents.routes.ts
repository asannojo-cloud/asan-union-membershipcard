...
const applySchema = z.object({
  comment: z.string().trim().min(1, "신청사유를 입력해주세요.").max(500),
});
...
memberEventsRouter.post("/:id/apply", async (req, res) => {
  ...
  const { rows } = await pool.query(`SELECT id, status FROM union_events WHERE id = $1`, [id]);
  const event = rows[0];
  ...
  await pool.query(
    ...
    [id, req.session.auth!.id, parsed.data.comment]
  );
  ...
  await recordAudit({
    ...
    action: "event_apply",
    newValue: { eventId: id, comment: parsed.data.comment },
  });
